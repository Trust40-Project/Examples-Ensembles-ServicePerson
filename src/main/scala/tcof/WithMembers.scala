...
trait WithMembers[+MemberType] extends WithConfig with Initializable {
  ...
  def membersWithSelectionIndicator: IndexedSeq[(Boolean, MemberType)] = {
    val selection = _solverModel.solution.getSetVal(allMembersVar)
    val values = allMembers.values.toIndexedSeq
    values.zipWithIndex.map{case (member, idx) => (selection.contains(idx), member)}
  }

  def selectedMembers: IndexedSeq[MemberType] = {
    val values = allMembers.values.toIndexedSeq
